...
def funcion_objetivo(prescriptive_concepts, individuo):        
    ...
    weight_mat = np.genfromtxt("fcmm_weight_diabetes.csv", delimiter=',')
    ...
    initial_vector = [individuo[0]] + prescriptive_concepts + individuo[3:8] + [individuo[8]]
    ...
    target = [dataset_stable_state_vector[8]]
    system_concepts = [dataset_stable_state_vector[0]] +  dataset_stable_state_vector[3:8] + target
    
    
    prv_fcm_stable_state_vector = results.tail(1).values.flatten().tolist()
    target_prv_fcm = [prv_fcm_stable_state_vector[8]]
    systems_concepts_prv_fcm = [prv_fcm_stable_state_vector[0]] +  prv_fcm_stable_state_vector[3:8]  + target_prv_fcm
...
def setup(individuo):
    ...
    toolbox.register("population", tools.initRepeat, list, 
                    toolbox.generate_prescriptive_concepts, 180)
...
def main(toolbox):
    cxpb, mutpb, ngen = 0.50, 0.30, 30#define the probabilities and generations number
...
if __name__ == "__main__":
    df = read_csv('test_diabetes_final_data.csv', delimiter = ",", header=None, index_col=False)
    ...
    for individuo in individuos:
        ...
        t2 = time.time()
        final_result.to_csv("diabetes_valores_prescriptivos.csv", header=None, index = False, mode='a')
